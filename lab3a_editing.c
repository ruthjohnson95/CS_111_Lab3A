...
ext2_super_bloc superblock; 

//struct Superblock superblock; 

struct ext2_group_desc* group_arr; // hold array of groups' info  
...
void print_to_csv()
{
  ...
  dprintf(file,"SUPERBLOCK,%d,%d,%d,%d,%d,%d,%d\n",superblock.n_blocks,superblock.n_inodes,superblock.block_size,superblock.inode_size,superblock.blocks_per_group,
	  superblock.inodes_per_group,superblock.nr_inode);
...
void superblock_summary()
{
  ...
  superblock.s_blocks_count = buffer_32;

  /* total inodes */
  pread(fd, &buffer_32, 4, superblock_offset + 0);
  superblock.s_inodes_count = buffer_32; 
...
void group_summary()
{
  ...
  group_arr = malloc(n_groups*sizeof(struct ext2_group_desc));
  ...
  for(i=0; i<n_groups; i++)
    {
      ...
      int num_blocks;
      double leftover_blocks = n_groups - ((double)superblock.s_blocks_count / superblock.s_blocks_per_group);
	...
	{
	  num_blocks = superblock.s_blocks_per_group - superblock.s_blocks_per_group * leftover_blocks; 
      ...
      int num_inodes;
      double leftover_inodes = ((double)superblock.s_inodes_count / superblock.s_inodes_per_group);
      if(leftover_inodes == 0)
	{
	  num_inodes = superblock.s_inodes_per_group; 
	}
      else
	{
	  num_inodes = superblock.s_inodes_per_group * leftover_inodes; 
	}
...
void free_blocks_summary()
{
  ...
  for(i=0; i<n_groups; i++)
	...
	{
	  int offset = group.bg_block_bitmap * superblock.s_blocks_count + j;
	  ...
	  for(k=1; k<=8; k++)
	      ...
	      if(result == 0)
		{
		  ...
		  int block_num = (j*8)+k+(i*superblock.s_blocks_count); 
  ...
  for(i=0; i<n_groups; i++)
    {
      ...
      for(j=0; j < superblock.s_blocks_count ;j++)
        {
          int offset = group.bg_inode_bitmap * superblock.s_blocks_count + j;
          pread(fd, &buffer_8, 1, offset);
          int8_t bitmask = 1;
          // iterate through the byte
          int k;
